...
# Phase 6.13 — Hardcoded Core 6-Card Grid Alignment
## Prompt 6.14 — Overwriting Dashboard Grid with Explicit Authentic Cards
Where to paste: Your VS Code AI Composer panel (Ctrl + I) in Agent/Project mode.

Intent: Strips out dynamic placeholder variables and explicitly codes the precise 6-card dashboard layout matching your platform structure with a clean off-white canvas.

### AI Prompt
Please open `src/components/sections/ForwardMovesHome.jsx`. We need to rewrite this file to implement our explicit 6-card matrix, renaming the core dynamic card from 'Reskilling' or 'Skill Studio' and ensuring there are absolutely zero yellow utility background leaks anywhere in the code.
...
Intent: Generates a dual-column workspace file layout embedding your comprehensive production link registry tracks, searchable side-filters, and clean text strings.

### AI Prompt
Please build a brand new isolated view component file exactly at `src/components/sections/SkillStudioView.jsx`. This page contains our interactive reskilling curriculum data. 

Populate the file with this clean, high-performance React code structure:
...
## Prompt 7.2 — Connecting State Switches inside App Router Shell
Where to paste: Your VS Code AI Composer panel (Ctrl + I) in Agent/Project mode.

Intent: Integrates the single-page view router condition inside src/App.jsx and wires layout properties seamlessly.
### AI Prompt
Please open `src/App.jsx`. We need to register our new `SkillStudioView` module into our core routing framework layout. 

Modify the component state structure to run this clean framework, ensuring no other parts are dropped or overwritten:
...
Intent: Ensures the global navigation bar renders a single dynamic button for 'SKILL STUDIO' when accessing internal project view blocks.

### AI Prompt
Please open `src/components/layout/Navbar.jsx`. We need to make sure that when a user moves off our main home landing page into the project sections (like `forward-moves` or `skill-studio`), the navbar navigation header link dynamically accommodates a pristine toggle tab point for our studio modules.

Please update the desktop link grouping AND the mobile hamburger overlay drawer layer to render an item reading: "SKILL STUDIO". Tapping this item must cleanly update our routing state:
...
Ensure that clicking the text logo on the left remains configured to return the screen safely back to 'home' at all times.

## Repository Sync Checkpoint
git add .
git commit -m "Deploy pristine interactive Skill Studio module view with clean sticky navbar routing links"
